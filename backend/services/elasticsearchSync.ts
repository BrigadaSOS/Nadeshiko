...
/**
 * Delete all segments for a specific media from ES
 */
...
/**
 * Find or create a sync log entry for a segment
 */
...
/**
 * Sync a segment to Elasticsearch. Only logs failures.
 * On retry success, the log entry is deleted.
 */
...
/**
 * Reindex segments for a specific media with optional season/episode filters.
 * Strategy: Delete all matching segments from ES first, then reindex from PostgreSQL.
...
/**
 * Reindex the entire database to ES.
 * Strategy: Process media by media.
 * For each media: delete all ES documents, then reindex from PostgreSQL.
 */
