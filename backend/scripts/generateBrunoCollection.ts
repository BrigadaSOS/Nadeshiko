...
interface PostResponseScript {
  // key is the URL path (e.g., '/v1/auth/login')
  [urlPath: string]: string;
}

// Pre-request script for JWT cookie auth
const JWT_PRE_REQUEST_SCRIPT = `const jwtToken = bru.getEnvVar("jwtToken");
if (jwtToken) {
  req.setHeader("Cookie", "access_token=" + jwtToken);
}`;

// Post-response scripts to inject into specific endpoints
const POST_RESPONSE_SCRIPTS: PostResponseScript = {
  '/v1/auth/login': `if (res.getStatus() >= 200 && res.getStatus() < 300) {
    const json = res.getBody();
    if (json.token) {
        bru.setEnvVar("jwtToken", json.token);
    }
}`,
  '/v1/auth/discord': `if (res.getStatus() >= 200 && res.getStatus() < 300) {
    const json = res.getBody();
    if (json.token) {
        bru.setEnvVar("jwtToken", json.token);
    }
}`,
  '/v1/auth/google': `if (res.getStatus() >= 200 && res.getStatus() < 300) {
    const json = res.getBody();
    if (json.token) {
        bru.setEnvVar("jwtToken", json.token);
    }
...
function writeRequestBruFile(folderPath: string, item: BrunoItem, seq: number): void {
  ...
  // Headers section - skip access_token header for JWT auth (handled by auth:apikey cookie)
  if (request.headers && request.headers.length > 0) {
    const filteredHeaders = request._isJwtAuth
      ? request.headers.filter((h: any) => h.name !== 'access_token')
      : request.headers;
  ...
  // Only output auth:apikey block for non-JWT endpoints
  if (request.auth?.mode === 'apikey' && !request._isJwtAuth) {
    content += `auth:apikey {\n`;
  ...
  // Add pre-request script for JWT auth endpoints
  if (request._isJwtAuth) {
    content += `\n`;
    content += `script:pre-request {\n`;
    const lines = JWT_PRE_REQUEST_SCRIPT.split('\n');
    for (const line of lines) {
      content += `  ${line}\n`;
    }
    content += `}\n`;
  ...
  // Add post-response script if one exists for this endpoint
  const cleanUrl = request.url.replace('{{baseUrl}}', '');
  const script = POST_RESPONSE_SCRIPTS[cleanUrl];
  if (script) {
    content += `\n`;
    content += `script:post-response {\n`;
    // Split script by newlines and indent each line
...
function writeFolderBruFile(folderPath: string, item: BrunoItem): void {
  ...
  // Note: User and AuthJwt folders use pre-request scripts for JWT cookie auth
...
function detectJwtAuth(openApiSpec: OpenAPISpec, request: any): void {
  const cleanUrl = request.url.replace('{{baseUrl}}', '');
      ...
      if (operation?.security) {
        for (const securityItem of operation.security) {
          for (const [schemeName] of Object.entries(securityItem)) {
            ...
            if (scheme?.in === 'cookie') {
              // Flag as JWT auth - will use pre-request script instead of auth:apikey
              request._isJwtAuth = true;
              return;
            }
          }
        }
...
function processItems(
  ...
  for (const item of items) {
    if (item.type === 'folder') {
      ...
      detectJwtAuth(openApiSpec, request);
