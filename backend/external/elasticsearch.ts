...
async function validateIndexSchema(): Promise<void> {
  try {
    const existingMappings = await client.indices.getMapping({ index: INDEX_NAME });
    const expectedBody = buildIndexBody();

    const existingProperties = existingMappings[INDEX_NAME]?.mappings?.properties as Record<string, unknown> | undefined;
    const expectedProperties = expectedBody.mappings.properties as Record<string, unknown>;

    if (!existingProperties) {
      logger.warn(`Elasticsearch index '${INDEX_NAME}' has no mappings defined`);
      return;
    }

    const expectedFields = Object.keys(expectedProperties);
    const existingFields = Object.keys(existingProperties);

    const missingFields = expectedFields.filter((field) => !existingFields.includes(field));
    if (missingFields.length > 0) {
      logger.warn(`Elasticsearch index '${INDEX_NAME}' is missing fields: ${missingFields.join(', ')}`);
      logger.warn('The schema may have been updated. Consider recreating the index to apply the new schema.');
    }

    const extraFields = existingFields.filter((field) => !expectedFields.includes(field));
    if (extraFields.length > 0) {
      logger.warn(`Elasticsearch index '${INDEX_NAME}' has unexpected fields: ${extraFields.join(', ')}`);
    }

    logger.info(`Elasticsearch index '${INDEX_NAME}' schema validated`);
  } catch (error) {
    logger.error(`Failed to validate index schema: ${error}`);
  }
}
...
export async function initializeElasticsearchIndex(): Promise<void> {
  ...
  if (indexExists) {
    logger.info(`Elasticsearch index '${INDEX_NAME}' already exists. Validating schema...`);
    await validateIndexSchema();
    return;
  }

  logger.info(`Creating Elasticsearch index '${INDEX_NAME}' with mappings from config/elasticsearch-schema.json`);

  const indexBody = buildIndexBody();
