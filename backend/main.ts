...
async function readAnimeDirectories(baseDir: string) {
  ...
  await Promise.all(
    animeDirectories.map(async (animeItem: string) => {
      const animeDirPath = path.join(baseDir, animeItem);

      // Se añaden los nombres de los animes a la base de datos
      ...
      if (fs.statSync(animeDirPath).isDirectory()) {
        // Leer los directorios dentro de la carpeta de anime
        const temporadaDirectories = fs.readdirSync(animeDirPath);

        await Promise.all(
          temporadaDirectories.map(async (tempItem: string) => {
            ...
            if (fs.statSync(tempDirPath).isDirectory()) {
              // Leer los directorios dentro de la carpeta de la temporada
              const episodeDirectories = fs.readdirSync(tempDirPath);

              await Promise.all(
                episodeDirectories.map(async (episodeItem: string) => {
                  ...
                  // Leer todos los elementos dentro del directorio del episodio
                  ...
                  if (dataCsvExists) {
                    ...
                    // Leer el archivo CSV fila por fila
