...
export const reindexSegment = async (req: any, res: Response, next: NextFunction) => {
  try {
    const { segmentId } = req.params;
    if (!segmentId || isNaN(Number(segmentId))) {
      return void res.status(StatusCodes.BAD_REQUEST).json({ error: 'Invalid segment ID' });
    }

    const success = await reindexSegmentById(Number(segmentId));
    if (success) {
      return void res.status(StatusCodes.OK).json({ message: `Segment ${segmentId} reindexed to Elasticsearch` });
    } else {
      return void res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: `Failed to reindex segment ${segmentId}` });
    }
  } catch (error) {
    next(error);
  }
...
export const reindexMediaSegmentsController = async (req: any, res: Response, next: NextFunction) => {
  try {
    const { mediaId } = req.params;
    const { season, episode } = req.body;

    if (!mediaId || isNaN(Number(mediaId))) {
      return void res.status(StatusCodes.BAD_REQUEST).json({ error: 'Invalid media ID' });
    }
    ...
    if (options.season !== undefined && isNaN(options.season)) {
      return void res.status(StatusCodes.BAD_REQUEST).json({ error: 'Invalid season number' });
    }
    if (options.episode !== undefined && isNaN(options.episode)) {
      return void res.status(StatusCodes.BAD_REQUEST).json({ error: 'Invalid episode number' });
    }
...
export const reindexFullDatabaseController = async (_req: any, res: Response, next: NextFunction) => {
  try {
    // Generate a unique reindex ID
...
export const getReindexStatus = async (_req: any, res: Response, next: NextFunction) => {
  try {
