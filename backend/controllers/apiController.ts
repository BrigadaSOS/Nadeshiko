...
export const createAPIKeyDefault = async (req: Request, res: Response) => {
  ...
  if (!requestedPermissions || requestedPermissions.length === 0) {
    ...
    if (hasNonReadPermission) {
      ...
      if (!roles.includes(1)) {
        throw new Forbidden('Only admin users can create API keys with permissions other than READ_MEDIA.');
      }
    }
    ...
    if (invalidPermissions.length > 0) {
      throw new BadRequest(`Invalid permissions: ${invalidPermissions.join(', ')}`);
    }
  }
...
export const listAPIKeysByUser = async (req: Request, res: Response) => {
  const { user_id } = req.jwt;
