...
const getContextSentence = async (item) => {
  ...
  // Fetch data from API      
  try {
    const response = await apiSearch.getContextSentence(body);
    ...
    <div
      ...
      <div
        ...
        <div class="flex justify-between items-center py-3 px-4 border-b dark:border-modal-border">
          <h3 class="font-bold text-gray-800 dark:text-white"> {{ t('searchpage.modalcontext.labels.context') }} - </h3>
          <button type="button"
        ...
        <div class="flex justify-end items-center gap-x-2 py-3 px-4 border-t dark:border-modal-border">
          <button type="button" data-hs-overlay="#hs-vertically-centered-scrollable-context"
