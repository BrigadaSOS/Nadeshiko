...
- **Anki**: a field per dictionary. Alongside "Definition", which is your whole stack, the placeholder menu now lists the dictionaries you have linked by name, so a note type can keep 三省堂 in one field and JMdict in another. There is also a "first dictionary" placeholder that needs no naming and follows whatever you move to the top of your stack.
- **Anki**: choose which dictionaries a card is about. When a word card is built from more than one of them, clicking a dictionary's name above its senses ticks it, and the note is written from what you ticked — the definition field, the first-dictionary field and the per-dictionary fields all follow the same pick. Select all and Deselect all both mean the whole stack, so a pick is one click from being undone, and it is dropped whenever the card moves to another word.
