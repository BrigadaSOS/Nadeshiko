
::about

# About

## Contribution and Attribution

<div class="about-contributors-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
  <div class="about-contributor-card border dark:bg-neutral-800 dark:border-neutral-700 overflow-hidden">
    <div class="flex items-start gap-4 p-4 border-b dark:border-neutral-700">
      <img
        class="w-32 h-32 rounded-lg object-cover flex-shrink-0"
        src="../github/natsume_pfp.jpg"
        alt="Natsume"
      />
      <div>
        <div class="text-xl font-bold text-gray-800 dark:text-white">Natsume</div>
        <div class="text-xs text-gray-500 dark:text-neutral-500 mt-0.5">Founder</div>
        <a href="https://github.com/Natsume-197" class="mt-2 inline-flex text-gray-400 hover:text-gray-500">
          <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path fill-rule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clip-rule="evenodd" />
          </svg>
        </a>
      </div>
    </div>
    <div class="px-4 pt-2 pb-2">
      <p class="text-xs text-gray-500 dark:text-neutral-400">Developer in charge of visual aspects, backend, and maintenance.</p>
    </div>
  </div>

  <div class="about-contributor-card border dark:bg-neutral-800 dark:border-neutral-700 overflow-hidden">
    <div class="flex items-start gap-4 p-4 border-b dark:border-neutral-700">
      <img
        class="w-32 h-32 rounded-lg object-cover flex-shrink-0"
        src="../github/davafons_pfp.jpg"
        alt="Davafons"
      />
      <div>
        <div class="text-xl font-bold text-gray-800 dark:text-white">Davafons</div>
        <div class="text-xs text-gray-500 dark:text-neutral-500 mt-0.5">Founder</div>
        <a href="https://github.com/davafons" class="mt-2 inline-flex text-gray-400 hover:text-gray-500">
          <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path fill-rule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clip-rule="evenodd" />
          </svg>
        </a>
      </div>
    </div>
    <div class="px-4 pt-2 pb-2">
      <p class="text-xs text-gray-500 dark:text-neutral-400">Developer in charge of Nadeshiko deployment, backend and Elasticsearch configuration.</p>
    </div>
  </div>

  <div class="about-contributor-card border dark:bg-neutral-800 dark:border-neutral-700 overflow-hidden">
    <div class="flex items-start gap-4 p-4 border-b dark:border-neutral-700">
      <img
        class="w-32 h-32 rounded-lg object-cover flex-shrink-0"
        src="../github/xyaman_pfp.jpg"
        alt="Xyaman"
      />
      <div>
        <div class="text-xl font-bold text-gray-800 dark:text-white">Xyaman</div>
        <div class="text-xs text-gray-500 dark:text-neutral-500 mt-0.5">Main Contributor</div>
        <a href="https://github.com/xyaman" class="mt-2 inline-flex text-gray-400 hover:text-gray-500">
          <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path fill-rule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clip-rule="evenodd" />
          </svg>
        </a>
      </div>
    </div>
    <div class="px-4 pt-2 pb-2">
      <p class="text-xs text-gray-500 dark:text-neutral-400">Contributor to the implementation of Anki with Nadeshiko.</p>
    </div>
  </div>
</div>

<br/>

## Credits and Data Sources

This site uses materials and references that are the intellectual property of their respective authors and rights holders. [See here](https://nadeshiko.co/media) for the list of content used as well as their respective authors.
<br/><br/>
For information regarding copyright and DMCA, please see our [DMCA Policy](/dmca).

## Contact

If you wish to contact us either to leave a comment, suggestion, contribution, or anything else, you can do so via:

- **Email**: contact@nadeshiko.co
- **Discord**: [Brigada SOS!団](https://discord.gg/ajWm26ADEj)

This website is under the AGPL-3.0 License.
