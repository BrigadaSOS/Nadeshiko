...
describe('sharedCdnMaxAge', () => {
  it.each(['en', 'es', 'ja', 'zh-CN', 'id', 'pt-BR'])('gives sentence permalinks the long TTL under /%s', (locale) => {
    expect(prod(`/${locale}/sentence/wy1hTtMJg6Jf`)).toBe(SHARED_CDN_MAX_AGE_SENTENCE);
  });

  it.each(['en', 'es', 'ja', 'zh-CN', 'id', 'pt-BR'])('gives search the long TTL under /%s', (locale) => {
