...
<template>
  ...
  <div class="fixed right-6 bottom-6 z-50 group">
    ...
    <button type="button" v-if="showScrollButton" @click="scrollToTop"
      class="flex items-center justify-center outline-none mb-2  bg-sgray rounded-full w-14 h-14 hover:bg-sgrayhover dark:bg-sred focus:ring-4 focus:outline-none">
      <BaseIcon :path="mdiArrowUp" w="5" h="5" size="20" fill="white" strokewidth="1" stroke="white" />
    </button>
  </Transition>
    <button type="button" id="drawer-button" data-drawer-target="drawer-navigation" data-drawer-show="drawer-navigation"
    ...
    <div class="relative lg:w-11/12 mx-auto mt-4">
      <div class="hs-dropdown relative inline-block w-full z-40">
        <button id="hs-dropdown-default" type="button"
          ...
          <svg aria-hidden="true" class="w-6 mx-2 fill-white hover:fill-gray-400 text-white dark:text-white"
          ...
          <div>
            ...
            <span v-if="props.type_sort === 'asc'">({{ t('searchpage.main.buttons.sortlengthmin') }})</span>
            <span v-else-if="props.type_sort === 'desc'">({{ t('searchpage.main.buttons.sortlengthmax') }})</span>
          </div>

          <svg class="hs-dropdown-open:rotate-180 w-2.5 h-2.5 text-white" width="16" height="16" viewBox="0 0 16 16"
        ...
        <div
          ...
          <a class="flex cursor-pointer items-center gap-x-3.5 py-2 px-3 rounded-md text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-sgrayhover dark:hover:text-gray-300"
            @click="fired_sort_length('none')">
          ...
          <a class="flex items-center cursor-pointer gap-x-3.5 py-2 px-3 rounded-md text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-sgrayhover dark:hover:text-gray-300"
    ...
    <div v-if="sentences.length > 0" class="lg:flex flex-col overflow-y-auto py-6">
      <ul id="unique-animes"
        ...
        <li v-for="item in filteredAnimes">
          <button @click="fired(item.anime_id)"
