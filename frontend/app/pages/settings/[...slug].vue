...
const tabs_advanced = [{ name: t('accountSettings.tabs.developer'), icon: mdiCodeTags, route: '/settings/developer' }];
                ...
                <nav aria-label="Tabs" class="flex flex-col dark:bg-card-background rounded-lg p-6 my-2 space-y-2">
                    ...
                    <button v-for="tab in tabs_general" :key="tab.name"
                        :class="{ 
                            ...
                            'opacity-50 cursor-not-allowed': !isAuthReady && tab.name === $t('accountSettings.tabs.account') 
                        }"
                        :disabled="!isAuthReady && tab.name === $t('accountSettings.tabs.account')"
