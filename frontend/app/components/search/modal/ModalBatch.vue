...
const wordsFound = computed(() => {
  return sortedWordsMatch.value.filter((item) => item.isMatch).length;
});
...
<template>
              ...
              <div class="px-4 sm:px-6 overflow-y-auto">
                <div class="space-y-4">
                  ...
                  <div
                    ...
                    <label for="nd-account-activity" class="flex p-4 md:p-5">
  ...
  <div id="nd-vertically-centered-scrollable-batch2"
    class="nd-overlay nd-overlay-backdrop-open:bg-neutral-900/40 hidden w-full h-full fixed top-0 left-0 z-[60] overflow-x-hidden overflow-y-auto">
    <div
      class="justify-center nd-overlay-open:opacity-100 nd-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all lg:max-w-3xl m-3 sm:mx-auto flex flex-col h-[calc(100%-3.5rem)] min-h-[calc(100%-3.5rem)]">
      <div
        ...
        <div class="flex justify-between items-center py-3 px-4 border-b dark:border-modal-border">
          ...
          <button type="button"
            class="nd-dropdown-toggle inline-flex flex-shrink-0 justify-center items-center h-8 w-8 rounded-md text-gray-500 hover:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 focus:ring-offset-white transition-all text-sm dark:focus:ring-gray-700 dark:focus:ring-offset-gray-800"
            data-nd-overlay="#nd-vertically-centered-scrollable-batch2">
            <span class="sr-only">{{ t('modalBatch.closeSrOnly') }}</span>
            <svg class="w-3.5 h-3.5" width="8" height="8" viewBox="0 0 8 8" fill="none"
              xmlns="http://www.w3.org/2000/svg">
              <path
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-white/30">
                      <tr v-for="(item, index) in sortedWordsMatch">
                        ...
                        <td
                          ...
                          <span :class="{
                            'bg-green-500/20 text-white': item.isMatch,
                            'bg-red-500/20 text-white': !item.isMatch
                          }"
                            ...
                            {{ item.isMatch ? t("batchSearch.results.true") : t("batchSearch.results.false") }}
                          </span>
                        </td>
