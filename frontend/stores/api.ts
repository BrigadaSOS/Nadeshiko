...
export const apiStore = defineStore('api', {
    actions: {
        async getApiKeysByUser(): Promise<ApiResponse | void> {
            ...
            try {
                const response = await fetch(`${config.public.backendUrl}/v1/user/getApiKeys`, {
                    method: "POST",
                    mode: "cors",
                    headers: {
        ...
        async deactivateApiKey(apiKeyId: string): Promise<ApiResponse | void> {
            const config = useRuntimeConfig();
            try {
                const response = await fetch(`${config.public.backendUrl}/v1/user/deactivateApiKey`, {
                    method: "POST",
                    mode: "cors",
                    headers: {
                        "Content-Type": "application/json"
        ...
        async createApiKeyGeneral(nameApiKey: string, permissions?: string[]): Promise<ApiResponse | void> {
            const config = useRuntimeConfig();
            try {
                const response = await fetch(`${config.public.backendUrl}/v1/user/createApiKey`, {
                    method: "POST",
                    mode: "cors",
                    headers: {
                        "Content-Type": "application/json"
