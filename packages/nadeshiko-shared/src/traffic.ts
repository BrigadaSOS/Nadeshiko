 ...
 *
 * ONE COPY, READ BY BOTH RUNTIMES. The frontend classifies the visitor at the
 * edge and the backend has to agree for the requests SSR proxies inward, so the
 * two have to give the same answer. They used to do that by holding two
 * byte-identical copies of this file and a test that failed the build when they
 * drifted; the copies, the hand-copy protocol and the drift test are all gone.
 *
 * Nothing here touches a `node:` module, deliberately: the frontend bundles this
 * into its client build as well, through `frontend/shared/utils/traffic.ts`.
 */
